...
async function enforceAuthGuard() {
  try {
    ...
    console.warn(`${LOG_PREFIX} Auth check failed, redirecting to login.`)
  }

  const currentPath = wixLocation.path && wixLocation.path.length ? `/${wixLocation.path.join('/')}` : '/add-new-patient'
...
$w.onReady(async function () {
  try {
    const isAuthenticated = await enforceAuthGuard()
    if (!isAuthenticated) {
      return
    }
